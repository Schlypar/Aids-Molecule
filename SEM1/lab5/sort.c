...
{
    if (end <= start) return (compare(key, &base[start]) > 0) ? (start + 1) : start;

    int mid = (start + end)/2;

    if (compare(key, &base[mid]) == 0) return mid + 1;

    if (compare(key, &base[mid]) > 0) return binarySearch(base, key, mid + 1, end, compare);
