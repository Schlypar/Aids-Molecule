...
private:
	...
	void ResizeSegments()
	{
		...
		Size segmentMeanNormalised = (size / SEGMENT_NORM_SIZE < 2) ? SEGMENT_MIN_SIZE
									    : SEGMENT_NORM_SIZE - SEGMENT_MIN_SIZE;
		...
		while (current)
		{
			if (current->size < SEGMENT_MIN_SIZE)
			{
				...
				{
					for (Index i = 0; i < diffrence && !SegmentIsEmpty(maxFrom(current->prev, current->next));
						i++)
					{
...
public:
	...
	T& Get(const Index index) const override
	{
		...
		while (segment)
		{
			...
			if ((index >= indexFromStart && index <= indexFromStart + indexLengthOfSegment - 1)
