...
int main(int, char**)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
		{
			...
			// first debug-tree
			firstInterface->gui->ShowData({ 5.f, 24.f }, treeWindowWidth, 0, "Main", firstInterface->labels.type,
				firstInterface->labels.elements);

			// second debug-tree
			secondInterface->gui->ShowData({ 5.f, 24.f }, treeWindowWidth, 10 + treeWindowWidth, "Auxiliary",
				secondInterface->labels.type, secondInterface->labels.elements);
